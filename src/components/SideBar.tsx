const SideBar = (page: any) => {
	...
	function completedStep(pageNumber: number) {
		if (pageNumber === 0) {
			return (
	...
	return (
		<ol className='relative text-gray-500 border-s border-gray-200 '>
			<li className='mb-3 ms-6'></li>
			<li className='mb-6 ms-6'>
				{pageNumber === 0 && completedStep(0)}
				{pageNumber !== 0 && defaultStep(0)}
				<h3
					className={`font-medium leading-tight ${
						pageNumber === 0 ? "text-blue-500 font-bold" : "text-gray-500"
					}`}
				>
				...
				<p
					className={`text-sm ${
						pageNumber === 0 ? "text-gray-600 font-medium" : ""
					}`}
				>
			...
			<li className='mb-8 ms-6'>
				{pageNumber === 1 && completedStep(1)}
				{pageNumber !== 1 && defaultStep(1)}
				<h3
					className={`font-medium leading-tight ${
						pageNumber === 1 ? "text-blue-500 font-bold" : "text-gray-500"
					}`}
				>
					Address Information
				</h3>
				<p
					className={`text-sm ${
						pageNumber === 1 ? "text-gray-600 font-medium" : ""
					}`}
				>
			...
			<li className='ms-6'>
				{pageNumber === 2 && completedStep(2)}
				{pageNumber !== 2 && defaultStep(2)}
				<h3
					className={`font-medium leading-tight ${
						pageNumber === 2 ? "text-blue-500 font-bold" : "text-gray-500"
					}`}
				>
					Account Information
				</h3>
				<p
					className={`text-sm ${
						pageNumber === 2 ? "text-gray-600 font-medium" : ""
					}`}
				>
					So you can log in.
				</p>
